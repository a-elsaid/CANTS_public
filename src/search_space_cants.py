...
class Inputs_Space:
    def __init__(
        ...
        for i, name in enumerate(inputs_names):
            self.inputs_spaces[name] = Single_Input_Space(
    ...
    def get_input(self, ant_exploration_rate: float) -> Point:
        pheromones = [
            input_space.input_pheromone for input_space in self.inputs_spaces.values()
        ]
        norm_pheromones = pheromones / np.sum(pheromones)
        input_name = np.random.choice(self.inputs_names, 1, p=norm_pheromones)[0]
        input_space = self.inputs_spaces[input_name]
        if np.random.random() < ant_exploration_rate or len(input_space.points) == 0:
            new_point = Point(
                0.0,
                0.0,
    ...
    def increase_pheromone(self, point: Point, pheromone_step: float) -> None:
        new_phermone_value = (
    ...
    def evaporate_pheromone(
        ...
        for input_space in self.inputs_spaces.values():
            max(input_space.input_pheromone * self.evaporation_rate, self.min_pheromone)
...
class Outputs_Space:
    def __init__(self, outputs_names):
        self.points = [
            Point(i / 10.0, 1.0, 0, np.random.random(), NODE_TYPE["OUTPUT"], name, i)
    ...
    def evaporate_pheromone(
        ...
        for point in self.points:
            max(point.pheromone * EVAPORATION_RATE, MIN_PHEROMONE)
            if point.pheromone < 0.05:
                points_to_be_removed.append(point)
        for pnt in points_to_be_removed:
            self.points.pop(pnt)

    def increase_pheromone(self, pnt: Point, pheromone_step) -> None:
...
class RNNSearchSpace:
    def __init__(
        ...
        self.output_space = Outputs_Space(outs_names)

    def evaporate_pheromone(
        self,
    ) -> None:
        self.input_space.evaporate_pheromone()
